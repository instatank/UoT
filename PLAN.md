...
**Current branch state (2026-07-13):** `main` now includes, in merge order: the atmosphere pass (2026-07-12, merged from `claude/atmosphere-interactive-elements-oa2to9` for planning-session context handover, not sign-off), then — via the integration branch `claude/merge-mobile-retreat-main-2vmyjz` (PR #1, 2026-07-13, at AA's explicit request) — the **mobile polish pass** (`claude/mobile-version-polish-n5tmks`: phone-viewport fixes across Voyage/maps/Door/crisis) and the **Retreat + character pass** (`claude/wellness-retreat-ui-design-oeg8mp`: the fifth presentation at `/retreat/[id]`), and finally the **Retreat mobile pass** (built directly on the integration branch after the merge exposed that the two sibling branches had never seen each other). All of it auto-deploys to production. The only merge conflict in the integration was NOTES.md (two build-log entries, both kept); `globals.css`/`SessionView`/`VoyageView` were touched by both branches in non-overlapping regions. AA's visual/on-device verdict on everything since the Voyage sign-off (atmosphere pass, mobile polish, Retreat, retreat mobile pass) is still outstanding; if changes are needed they land as new work on `main`, not branch reverts. No other branches carry unmerged work.
